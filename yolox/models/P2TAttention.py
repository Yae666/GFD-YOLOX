...
class P2TAttention(nn.Module):
    ...
    def forward(self, x, H, W, d_convs=None):
        ...
        q = self.q(x).reshape(B, N, self.num_heads, torch.div(C, self.num_heads, rounding_mode='floor')).permute(0, 2,
                                                                                                                 1, 3)
        ...
        for (pool_ratio, l) in zip(self.pool_ratios, self.d_convs1):
            ...
            pool = F.adaptive_avg_pool2d(x_, (
            torch.div(H, pool_ratio, rounding_mode='floor'), torch.div(W, pool_ratio, rounding_mode='floor')))
        ...
        kv = self.kv(pools).reshape(B, -1, 2, self.num_heads,
                                    torch.div(C, self.num_heads, rounding_mode='floor')).permute(2, 0, 3, 1, 4)
...
if __name__ == '__main__':
    ...
    ### 对图像中的patch进行划分并编码 ###
    PatchEmbed = PatchEmbed(img_size=640, patch_size=20, kernel_size=20, in_chans=3, embed_dim=192, overlap=False)
